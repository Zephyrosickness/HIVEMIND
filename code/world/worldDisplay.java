...
public class worldDisplay{
    ...
    private static void addInfoToPanel(JPanel panel, JPanel scorePanel){
        ...
        //dropdown for operator on resultSteps count
        final JComboBox<String> stepOperator = new JComboBox<>(operators);
        stepOperator.setName("STEP Operator");

        //dropdown for operator on partnerStat count
        final JComboBox<String> partnerOperator = new JComboBox<>(operators);
        partnerOperator.setName("Partner Operator");
        ...
        //field for steps taken count
        final JTextField stepsTakenField = new JTextField("0");
        stepsTakenField.setVisible(false);
        stepsTakenField.setEnabled(false);
        stepsTakenField.setName("STEPs Taken");

        //field for lost count
        final JTextField partnerField = new JTextField("0");
        partnerField.setVisible(false);
        partnerField.setEnabled(false);
        partnerField.setName("Partner's Stat");
        ...
        Component[] itemsInOrder = new Component[]{
                new JLabel("FAR Count"), stepOperator, stepsTakenField,

                new JLabel("LOST Count"), partnerOperator, partnerField,
        ...
        stepOperator.addActionListener(_ -> {
            if(Objects.requireNonNull(stepOperator.getSelectedItem()).toString().equals("Any")){
                stepsTakenField.setEnabled(false);
                stepsTakenField.setVisible(false);
            }else{
                stepsTakenField.setEnabled(true);
                stepsTakenField.setVisible(true);
            }
        });

        partnerOperator.addActionListener(_ -> {
            if(Objects.requireNonNull(partnerOperator.getSelectedItem()).toString().equals("Any")){
                partnerField.setEnabled(false);
                partnerField.setVisible(false);
            }else{
                partnerField.setEnabled(true);
        ...
        run.addActionListener(_ -> {
            final double CC = getChart(Objects.requireNonNull(songSelect.getSelectedItem()).toString(), Objects.requireNonNull(difficultySelect.getSelectedItem()).toString()).getNum(ChartAttributes.CC);
            worldCalc.getAllPossibleRanges(
                Integer.parseInt(partnerField.getText()),
                Double.parseDouble(stepsTakenField.getText()),
                    CC,
                    partnerOperator.getSelectedItem().toString(),
                    stepOperator.getSelectedItem().toString());
                });
    }
    ...
    private static void runCalcs(JPanel scorePanel){
        ...
        final int partnerStat = (int)Double.parseDouble(getComponentValue("Lost Count"));
        ...
        //scoreCalc.calcAllPossibleScores(minScore, far, partnerStat, farOp, missOp, toaStatus, chart.getNum(ChartAttributes.CC), chart.getNum(ChartAttributes.NOTECOUNT));
    ...
    protected static class ScoreTextArea extends JTextArea{
        ...
        private ScoreTextArea(String text, double score, double resultSteps, int partnerStat) {
            //variables for each score container
            super(text);
            this.score = score;
            this.resultSteps = resultSteps;
            this.partnerStat = partnerStat;
        ...
        private int getFar(){return (int)resultSteps;}
        private int getMiss(){return (int)partnerStat;}
    }

    protected static void importComponent(double score, double rating, int partner){
        //adds judgement data and score to array
        String text = "Score: " + (int) score + "\nSTEPs: " + rating + "\nPartner Stat: " + partner;
        ScoreTextArea textArea = new ScoreTextArea(text, score, rating, partner);
